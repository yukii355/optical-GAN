...
vgg16 = nn.Sequential(*list(vgg16.features)[:23]).to(device)
for param in vgg16:
...
for epoch in range(num_epochs):
    ...
    for itr, (img1, img2, ani, flow) in enumerate(trainloader):
            ...
            l_real = L2distance(real_dis, real_label)
            l_fake = L2distance(fake_dis, fake_label)
            l_dis = ((l_real + l_fake) / 2)*10e4
            l_dis.backward(retain_graph=True)
            optimizerD.step()

            optimizerG.zero_grad()
            l_adv = L2distance(fake_dis, real_label)
            fake_vgg = vgg16(fake_style1)
            #print(fake_vgg.size())
            l_con = L2distance(fake_vgg, vgg16(img1))
            l_style = L2distance(vgg16(ani), fake_vgg)
            out_item = fake_style2 - warp(fake_style1, flow)
            in_item = img2 - warp(img1, flow)
            l_flow = L2distancematrix(out_item, in_item)

            l_gen = l_adv.mean() + l_con.mean() + l_style.mean() + 10e4*l_flow.mean()
